...
init_vault() {
    ...
    local vault_status=$(curl -s "$VAULT_URL/v1/sys/init" | jq -r '.initialized')
    
    if [[ "$vault_status" != "true" ]]; then
        log_info "Vault not initialized, initializing now..."
        
        # Initialize Vault
        local init_response=$(curl -s -X PUT "$VAULT_URL/v1/sys/init" \
...
store_secret() {
    ...
    # Store secret in Vault
    local response=$(curl -s -X POST "$VAULT_URL/v1/$VAULT_SECRETS_PATH/data/$secret_name" \
        -H "X-Vault-Token: $vault_token" \
...
retrieve_secret() {
    ...
    # Retrieve secret from Vault
    local response=$(curl -s -X GET "$VAULT_URL/v1/$VAULT_SECRETS_PATH/data/$secret_name" \
        -H "X-Vault-Token: $vault_token")
    ...
    local secret_value=$(echo "$response" | jq -r '.data.data.value')
...
rotate_secrets() {
    ...
    local secrets=$(jq -r '.secrets | keys[]' "$config_file")
    
    for secret_name in $secrets; do
        log_info "Rotating secret: $secret_name"
        
        # Get secret configuration
        local complexity=$(jq -r ".secrets.\"$secret_name\".complexity" "$config_file")
        
        # Generate new password
        local new_password=$(generate_password 32 "$complexity")
...
export_vault_secrets() {
    ...
    # List all secrets
    local secrets_list=$(curl -s -X LIST "$VAULT_URL/v1/$VAULT_SECRETS_PATH/metadata" \
    ...
    for secret_name in $secrets_list; do
        local secret_value=$(retrieve_secret "$secret_name" 2>/dev/null || echo "")
...
show_status() {
    ...
    if [[ -f "$SECURITY_DIR/secrets/secrets-config.json" ]]; then
        ...
        local secret_count=$(jq -r '.secrets | length' "$SECURITY_DIR/secrets/secrets-config.json")
