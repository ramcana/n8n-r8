...
backup_config() {
    ...
    cp """$BACKUP_PATH""/.env" """$BACKUP_PATH""/config/" 2>/dev/null || warning ".env file not found"
    cp """$BACKUP_PATH""/docker-compose.yml" """$BACKUP_PATH""/config/" 2>/dev/null || warning "docker-compose.yml not found"
    cp """$BACKUP_PATH""/docker-compose.nginx.yml" """$BACKUP_PATH""/config/" 2>/dev/null || warning "docker-compose.nginx.yml not found"
    cp """$BACKUP_PATH""/docker-compose.traefik.yml" """$BACKUP_PATH""/config/" 2>/dev/null || warning "docker-compose.traefik.yml not found"
    # Copy nginx config if exists
    if [[ -d """$BACKUP_PATH""/nginx" ]]; then
        cp -r """$BACKUP_PATH""/nginx" """$BACKUP_PATH""/config/"
    fi
    # Copy traefik config if exists
    if [[ -d "$1/traefik" ]]; then
        cp -r "$1/traefik" "$1/config/"
    fi
...
cleanup_old_backups() {
    ...
    find "$BACKUPS_DIR" -type d -name "n8n_backup_*" -mtime +"$retention_days" -exec rm -rf {} + 2>/dev/null || true
...
main() {
    ...
    create_metadata
    cleanup_old_backups "${RETENTION_DAYS:-7}"  # Default to 7 days if not set
