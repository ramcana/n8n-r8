...
detect_running_configurations() {
    ...
    local n8n_containers=$(docker ps --format "{{.Names}}" | grep -E "^n8n" || true)
...
show_running_services() {
    ...
    for compose_cmd in "${compose_files[@]}"; do
        local services=$(docker compose -f $compose_cmd ps --format "table {{.Name}}\t{{.Status}}\t{{.Ports}}" 2>/dev/null | tail -n +2 || true)
        if [[ -n "$services" ]]; then
            found_services=true
            echo "$services"
        fi
    ...
    local individual_containers=$(docker ps --format "table {{.Names}}\t{{.Status}}\t{{.Ports}}" | grep -E "^n8n" || true)
...
stop_individual_containers() {
    ...
    local n8n_containers=$(docker ps -q --filter "name=n8n" 2>/dev/null || true)
...
show_stop_summary() {
    ...
    local remaining_containers=$(docker ps --format "{{.Names}}" | grep -E "^n8n" || true)
    ...
    local remaining_networks=$(docker network ls --format "{{.Name}}" | grep -E "n8n" || true)
    if [[ -n "$remaining_networks" ]]; then
        info "Remaining networks:"
        echo "$remaining_networks" | sed 's/^/    /'
    fi
    
    local remaining_volumes=$(docker volume ls --format "{{.Name}}" | grep -E "n8n" || true)
...
main() {
    ...
    # Detect running configurations
    local configurations=($(detect_running_configurations))
