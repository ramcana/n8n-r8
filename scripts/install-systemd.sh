...
list_services() {
    ...
    for service_type in basic nginx traefik monitoring; do
        local service_file=$(get_service_file "$service_type")
        if [[ -f "$SYSTEM_SYSTEMD_DIR/$service_file" ]]; then
            local status=$(systemctl is-enabled "$service_file" 2>/dev/null || echo "disabled")
            local active=$(systemctl is-active "$service_file" 2>/dev/null || echo "inactive")
...
install_service() {
    ...
    local service_file=$(get_service_file "$service")
