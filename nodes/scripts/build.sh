...
build_nodes() {
    ...
    # Count built files
    local node_files=$(find dist -name "*.node.js" | wc -l)
    local credential_files=$(find dist -name "*.credential.js" | wc -l)
...
package_nodes() {
    ...
    local package_file=$(ls -t *.tgz | head -n1)
...
show_build_info() {
    ...
    if [[ -d "dist" ]]; then
        local node_count=$(find dist -name "*.node.js" | wc -l)
        local credential_count=$(find dist -name "*.credential.js" | wc -l)
