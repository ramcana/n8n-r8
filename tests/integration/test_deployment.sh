...
test_nginx_proxy_deployment() {
    ...
    # Test proxy headers
    local response_headers=$(curl -s -I "$TEST_NGINX_URL" 2>/dev/null || echo "")
...
test_deployment_persistence() {
    ...
    local file_content=$(cat "$test_data_file")
...
test_deployment_scaling() {
    ...
    local n8n_count=$(docker compose ps n8n --format "{{.Names}}" | wc -l)
...
test_deployment_resource_limits() {
    ...
    # Check memory usage
    local n8n_memory=$(docker stats --no-stream --format "{{.MemUsage}}" n8n-r8-n8n-1 2>/dev/null | cut -d'/' -f1 | sed 's/[^0-9.]//g' || echo "0")
