...
test_backup_directory_creation() {
    ...
    local mock_backup_script=$(cat << 'EOF'
#!/bin/bash
mkdir -p "$BACKUP_DIR"
...
EOF
    ...
    local services=$(jq -r '.services[]' "$metadata_file" | wc -l)
...
test_backup_size_calculation() {
    ...
    local human_size=$(du -h "$test_file" | cut -f1)
...
test_backup_rotation() {
    ...
    for i in {1..5}; do
        local old_date=$(date -d "$i days ago" '+%Y%m%d_%H%M%S')
        touch "$backup_dir/n8n_backup_${old_date}.tar.gz"
    done
    
    # Count files before rotation
    local files_before=$(ls "$backup_dir"/n8n_backup_*.tar.gz | wc -l)
    ...
    local files_after=$(ls "$backup_dir"/n8n_backup_*.tar.gz 2>/dev/null | wc -l)
...
test_backup_verification() {
    ...
    local checksum1=$(sha256sum "$test_backup" | cut -d' ' -f1)
    local checksum2=$(sha256sum "$test_backup" | cut -d' ' -f1)
