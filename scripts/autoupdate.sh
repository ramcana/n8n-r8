...
load_config() {
    ...
    AUTOUPDATE_ENABLED="${AUTOUPDATE_ENABLED:-false}"
...
check_lock() {
    if [[ -f "$LOCK_FILE" ]]; then
        ...
        if kill -0 "$pid" 2>/dev/null; then
            error_exit "Autoupdate script is already running (PID: $pid)"
        else
...
check_updates() {
    ...
    # Check n8n updates
    local current_n8n=$(docker images --format "table {{.Repository}}:{{.Tag}}\t{{.ID}}" | grep "n8nio/n8n:latest" | awk '{print $2}' || echo "")
    docker pull n8nio/n8n:latest >/dev/null 2>&1
    local latest_n8n=$(docker images --format "table {{.Repository}}:{{.Tag}}\t{{.ID}}" | grep "n8nio/n8n:latest" | awk '{print $2}' || echo "")
    
    if [[ "$current_n8n" != "$latest_n8n" && -n "$latest_n8n" ]]; then
        log "INFO" "N8N update available: $current_n8n -> $latest_n8n"
        updates_available=true
    fi
    
    # Check PostgreSQL updates
    local current_postgres=$(docker images --format "table {{.Repository}}:{{.Tag}}\t{{.ID}}" | grep "postgres:15-alpine" | awk '{print $2}' || echo "")
    docker pull postgres:15-alpine >/dev/null 2>&1
    local latest_postgres=$(docker images --format "table {{.Repository}}:{{.Tag}}\t{{.ID}}" | grep "postgres:15-alpine" | awk '{print $2}' || echo "")
    
    if [[ "$current_postgres" != "$latest_postgres" && -n "$latest_postgres" ]]; then
        log "INFO" "PostgreSQL update available: $current_postgres -> $latest_postgres"
        updates_available=true
    fi
    
    # Check Redis updates
    local current_redis=$(docker images --format "table {{.Repository}}:{{.Tag}}\t{{.ID}}" | grep "redis:7-alpine" | awk '{print $2}' || echo "")
    docker pull redis:7-alpine >/dev/null 2>&1
    local latest_redis=$(docker images --format "table {{.Repository}}:{{.Tag}}\t{{.ID}}" | grep "redis:7-alpine" | awk '{print $2}' || echo "")
...
rollback() {
    ...
    local backup_name=$(cat "${BACKUP_DIR}/.last_pre_update_backup")
