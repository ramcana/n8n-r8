...
check_updates() {
    ...
    # Check n8n updates
    local current_n8n
    current_n8n=$(docker images --format "table {{.Repository}}:{{.Tag}}\t{{.ID}}" | grep "n8nio/n8n:latest" | awk '{print $2}' || echo "")
    docker pull n8nio/n8n:latest >/dev/null 2>&1
    local latest_n8n
    latest_n8n=$(docker images --format "table {{.Repository}}:{{.Tag}}\t{{.ID}}" | grep "n8nio/n8n:latest" | awk '{print $2}' || echo "")
    ...
    current_postgres=$(docker images --format "table {{.Repository}}:{{.Tag}}\t{{.ID}}" | grep "postgres:15-alpine" | awk '{print $2}' || echo "")
    docker pull postgres:15-alpine >/dev/null 2>&1
    local latest_postgres
    latest_postgres=$(docker images --format "table {{.Repository}}:{{.Tag}}\t{{.ID}}" | grep "postgres:15-alpine" | awk '{print $2}' || echo "")
    ...
    # Check Redis updates
    local current_redis
    current_redis=$(docker images --format "table {{.Repository}}:{{.Tag}}\t{{.ID}}" | grep "redis:7-alpine" | awk '{print $2}' || echo "")
